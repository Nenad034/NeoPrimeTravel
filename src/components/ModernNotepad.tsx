...
import {
    ...
    Download, RefreshCw, Image as ImageIcon, Paperclip, Mail, Send, MessageCircle, Share2
} from 'lucide-react';
...
export const ModernNotepad: React.FC<ModernNotepadProps> = ({ onClose, isDark }) => {
    // State Management
    const [tabs, setTabs] = useState<Tab[]>(() => {
        const saved = localStorage.getItem('neo-notepad-tabs');
        if (saved) {
            try { 
    ...
    useEffect(() => {
        ...
        saveTimeout.current = setTimeout(() => {
            localStorage.setItem('neo-notepad-tabs', JSON.stringify(tabs));
        }, 1000);
    ...
    const shareNote = (platform: 'email' | 'whatsapp' | 'messages') => {
        let text = '';
        if (activeTab.mode === 'document') {
            text = activeTab.content.replace(/<[^>]+>/g, '');
        } else {
            text = activeTab.blocks.map(b => b.content).join('\n\n');
        }
        
        const subject = encodeURIComponent(activeTab.title);
    ...
    const exportAs = (format: 'word' | 'excel' | 'pdf' | 'html') => {
        let content = '';
        let htmlContent = '';
            ...
            if (file) {
                ...
                reader.onload = (re) => {
                    ...
                    if (type === 'image') {
                        ...
                        const link = `<a href="${result}" download="${file.name}" style="color: var(--bordo); text-decoration: underline;">Dokument: ${file.name}</a>`;
    ...
    const Dropdown: React.FC<{ items: { label: string; action: () => void; icon?: any }[] }> = ({ items }) => (
        <div style={{
            position: 'absolute', top: '100%', left: 0, background: isDark ? '#252526' : '#fff',
            border: isDark ? '1px solid #454545' : '1px solid #e2e8f0', borderRadius: '4px', zIndex: 100, minWidth: '160px',
    ...
    return createPortal(
        ...
        >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', height: '100%', overflowX: 'auto', scrollbarWidth: 'none' }}>
                    {tabs.map(tab => (
                        <div key={tab.id} onClick={() => setActiveTabId(tab.id)} style={{
                            ...
                            <FileText size={14} color={activeTabId === tab.id ? "var(--bordo)" : "#858585"} />
                            {activeTabId === tab.id ? (
                                <input 
                                    value={tab.title}
                                    onChange={(e) => {
            ...
            <div style={{
                ...
                {[
                    { label: 'File', items: [
                        ...
                        { label: 'Export to Excel', action: () => exportAs('excel'), icon: Grid },
                        { label: 'Export to PDF', action: () => exportAs('pdf'), icon: Download },
                        { label: 'Export to HTML', action: () => exportAs('html'), icon: FileText },
                        { label: 'Download (.txt)', action: saveToFile, icon: Download },
                    ] },
                    { label: 'Edit', items: [
                        { label: 'Find', action: () => setSearchOpen(true), icon: Search }, 
                    ...
                    { label: 'Share', items: [
                        { label: 'Send via Email', action: () => shareNote('email'), icon: Mail },
                        { label: 'Send via WhatsApp', action: () => shareNote('whatsapp'), icon: MessageCircle },
                        { label: 'Send as Message', action: () => shareNote('messages'), icon: Send },
                    ] },
                ].map(menu => (
                    <div key={menu.label} style={{ position: 'relative' }}>
                        <div
                            ...
                            style={{
                                ...
                                background: activeMenu === menu.label ? (isDark ? '#3e3e42' : '#f1f5f9') : 'transparent',
                                color: activeMenu === menu.label ? (isDark ? '#ffffff' : 'var(--bordo)') : (isDark ? '#cccccc' : '#334155'),
                                transition: 'all 0.2s'
                            }}>
                            {menu.label}
                        </div>
            ...
            <div style={{
                ...
                <div style={{ position: 'relative' }}>
                    ...
                    {formatMenuOpen && (
                        <div style={{
                            position: 'absolute', top: '100%', left: 0, background: isDark ? '#252526' : '#fff',
                            border: isDark ? '1px solid #454545' : '1px solid #e2e8f0', borderRadius: '4px', zIndex: 100, minWidth: '80px',
                            boxShadow: '0 4px 12px rgba(0,0,0,0.5)', padding: '4px 0'
                        }}>
                            {['H1', 'H2', 'H3', 'H4'].map(h => (
                                <div
                                    ...
                                    style={{ padding: '6px 16px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer', color: isDark ? '#ccc' : '#333' }}
                                    className="menu-item-hover"
                                >
                                    {h}
                                </div>
                ...
                <button onClick={saveToFile} style={{ background: 'transparent', border: 'none', color: 'var(--bordo)', cursor: 'pointer', padding: '6px' }} title="Download as .txt"><Download size={18} /></button>
            </div>
                                        ...
                                        {PASTEL_COLORS.map(c => (
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    boxShadow: block.color === c.color ? '0 0 0 2px var(--bordo)' : 'none'
                                                }}
                                            />
                                        ))}
                                        <div style={{ flex: 1 }} />
            ...
            <div style={{
                ...
                <div style={{ flex: 1 }} />
                <span>UTF-8</span>
                <span>Powered by NeoTravel AI</span>
            </div>
            <style>{`
                .menu-item-hover:hover { background: var(--bordo); color: #ffffff !important; }
